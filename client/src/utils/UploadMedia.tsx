      ...
      {media.preview ? (
        <div className="space-y-3">
            ...
            {media.file instanceof File && (
              <div className="bg-black4 border border-black5 rounded-lg p-3 flex items-center justify-between">
                <div className="flex items-center gap-3 flex-1 min-w-0">
                  {image ? (
                    <FaImage className="text-yellow8 text-xl flex-shrink-0" />
                  ) : video ? (
                    <FaVideo className="text-yellow8 text-xl flex-shrink-0" />
                  ) : (
                    <FaFile className="text-yellow8 text-xl flex-shrink-0" />
                  )}
          ...
          <button
            type="button"
            onClick={handleClick}
            className="w-full bg-black4 hover:bg-black3 border border-black5 hover:border-yellow8 text-white py-2.5 px-4 rounded-lg transition-all duration-200 flex items-center justify-center gap-2 text-sm font-medium"
          >
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-8 sm:p-12 text-center cursor-pointer transition-all duration-200 ${
            isDragging
              ? "border-yellow8 bg-yellow8/10 scale-[1.02]"
              : "border-black5 hover:border-yellow8 bg-black3 hover:bg-black4"
        ...
        >
          <div className="flex flex-col items-center justify-center gap-4">
            ...
            <div className={`p-4 rounded-full transition-all duration-200 ${
              isDragging 
                ? "bg-yellow8/20 scale-110" 
                : "bg-black4"
            }`}>
              {image ? (
                <FaImage className={`text-4xl ${isDragging ? "text-yellow8" : "text-yellow8"}`} />
              ) : video ? (
                <FaVideo className={`text-4xl ${isDragging ? "text-yellow8" : "text-yellow8"}`} />
              ) : (
                <FaUpload className={`text-4xl ${isDragging ? "text-yellow8" : "text-yellow8"}`} />
              )}
            </div>
            ...
            <button
              type="button"
              className="mt-2 bg-yellow8 hover:bg-yellow9 text-black font-semibold py-2.5 px-6 rounded-lg transition-all duration-200 hover:scale-105 shadow-lg"
            >
              Browse Files
            </button>
          </div>
